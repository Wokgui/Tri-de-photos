...
set "ICON=%LOCALAPPDATA%\Tri de photos\Tri de photos transparent v3.ico"
...
rem Conserve exactement la tuile arrondie, le logo et le texte.
rem Seul le fond blanc EXTERIEUR connecte aux quatre coins devient transparent.
rem Le nouveau nom du fichier ICO force Windows a charger cette version au lieu du cache precedent.
py -c "from PIL import Image,ImageDraw; import os; src=Image.open(os.environ['ICON_SRC']).convert('RGBA'); w,h=src.size; corners=[(0,0),(w-1,0),(0,h-1),(w-1,h-1)]; [ImageDraw.floodfill(src,p,(255,255,255,0),thresh=70) for p in corners]; src.save(os.environ['ICON'],format='ICO',sizes=[(256,256),(128,128),(96,96),(64,64),(48,48),(32,32),(24,24),(16,16)])"
if errorlevel 1 (
...
rem Supprime l'ancien raccourci du Bureau puis cree le nouveau avec le NOUVEAU chemin d'icone.
...
echo Le fond carre exterieur de l'icone a ete supprime.
pause
